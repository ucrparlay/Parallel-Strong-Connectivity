...
def run_SCC(graph_key, file_out, n_thread, cores):
...
def run_GBBS(graph_key, file_out, n_thread, cores):
    ...
    subprocess.call(cmd, shell=True)
def run_iSpan(graph_key, file_out, n_thread, cores):
...
def run_MultiStep(graph_key, file_out, n_thread, cores):
...
def run_scale(graph):
    ...
    # algorithms = {run_SCC:"scc", run_GBBS:"gbbs", run_iSpan:"ispan", run_MultiStep:"multistep"}
    algorithms = {run_MultiStep:"multistep"}
    for a, surfix in algorithms.items():
        ...
        for n_thread, cores in threads.items():
            ...
            try:
                ...
                key_words = "# time per iter: float(nan)" if (a == run_GBBS) else "average cost: float(nan)"
