...
class LDD {
 ...
 public:
  ...
  timer t_sparse_update;
  timer t_dense_update;
  timer t_sparse2dense;
  timer t_dense2sparse;
  timer t_judge;
  timer t_total;
...
#define print_time(name)                                                    \
  printf("%-15s running time: %10f, %10f%%\n", #name, t_##name.get_total(), \
         100. * t_##name.get_total() / t_total.get_total());
...
size_t LDD::sparse_update(sequence<NodeId>& label, sequence<NodeId>& parent) {
  ...
  parallel_for(
      ...
      1);
  auto ret = bag.pack(make_slice(front));
#ifdef BREAKDOWN
  ...
  parallel_for(
      ...
      [&](NodeId i) {
        ...
        if (label[i] == UINT_N_MAX) {
          const auto& offset = G.symmetric ? G.offset : G.in_offset;
          const auto& E = G.symmetric ? G.E : G.in_E;
#ifdef BREAKDOWN
...
tuple<sequence<NodeId>, sequence<NodeId>> LDD::ldd(double beta,
  ...
  // t0.stop();
  // cout << "perm time " << t0.get_total()<<endl;
  // BEGIN SAMPLE
  ...
  t0.stop();
#if defined(BREAKDOWN)
  cout << "sample time " << t0.get_total() << endl;
#endif
  // size_t num_added = 0;
